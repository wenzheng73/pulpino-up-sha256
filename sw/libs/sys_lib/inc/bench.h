...
/**
 * @brief Wait for the specified time and do nothing 
 * @param The delay time unit is us,
 * Time calibration depends on SOC_CLK under config.h 
 */
void ic_delay_us(unsigned int time);

/*Used to initialize some related peripherals, so that they can be used normally in the app */
void workspace_init();
#endif
